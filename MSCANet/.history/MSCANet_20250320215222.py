...
class MSCANet(nn.Module):
    ...
    def __init__(self,
        ...
        self.att4 = LFEM(512,512)
        self.att3 = LFEM(256,256)
        self.att2 = LFEM(128,128)
        self.att1 = LFEM(64,64)
        self.pre_conv = ConvBN(encoder_channels[-1], encoder_channels[-1], kernel_size=1)


        self.ff1 = ETB(in_channels=encoder_channels[-1], key_channels_redution=4, num_heads=16, window_size=16, with_pos=True)
        self.ff2 = ETB(in_channels=encoder_channels[-2], key_channels_redution=4, num_heads=8, window_size=16, with_pos=True)
        self.ff3 = ETB(in_channels=encoder_channels[-3], key_channels_redution=4, num_heads=4, window_size=16, with_pos=True)
        self.ff4 = ETB(in_channels=encoder_channels[-4], key_channels_redution=4, num_heads=2, window_size=16, with_pos=True)
...
if __name__ == '__main__':
    ...
    model = MSCANet(in_channels=3, num_classes=4)
    out = model(x)
    print(out.shape)
